...
/** @file binlogdb.h
    ...
    @brief
  The binlogdb engine is a stubbed storage engine for example purposes only;
  it does nothing at this point. Its purpose is to provide a source
  code illustration of how to begin writing new storage engines; see also
  /storage/example/binlogdb.cc.

    @note
  Please read binlogdb.cc before reading this file.
  ...
  *required* to be implemented. For a full list of all methods that you can
  implement, see handler.h.
   ...
   @see
  /sql/handler.h and /storage/example/binlogdb.cc
*/
...
/** @brief
  Binlogdb_share is a class that will be shared among all open handlers.
  This example implements the minimum of what you will probably need.
*/
class Binlogdb_share : public Handler_share {
 public:
  THR_LOCK lock;
  Binlogdb_share();
  ~Binlogdb_share() override { thr_lock_delete(&lock); }
};

/** @brief
  Class definition for the storage engine
*/
class binlogdb : public handler {
  THR_LOCK_DATA lock;          ///< MySQL lock
  Binlogdb_share *share;        ///< Shared lock info
  Binlogdb_share *get_share();  ///< Get the share
  Binlog_table_reader * binlog_table_reader;  ///< Reader for the table

 public:
  binlogdb(handlerton *hton, TABLE_SHARE *table_arg);
  ~binlogdb() override {}

  /** @brief
    The name that will be used for display purposes.
   */
  const char *table_type() const override { return "BINLOGDB"; }
  ...
  ulonglong table_flags() const override {
    ...
    */
    return (HA_NO_TRANSACTIONS | HA_NO_AUTO_INCREMENT | HA_BINLOG_ROW_CAPABLE |
            HA_BINLOG_STMT_CAPABLE | HA_CAN_REPAIR | HA_UPDATE_NOT_SUPPORTED |
  ...
  */
  ...
  /*
    Everything below are methods that we implement in binlogdb.cc.

    Most of these methods are not obligatory, skip them and
    MySQL will treat them as not implemented
  */
  /** @brief
    We implement this in binlogdb.cc; it's a required method.
  */
  ...
  */
  ...
  /** @brief
    We implement this in binlogdb.cc. It's not an obligatory method;
